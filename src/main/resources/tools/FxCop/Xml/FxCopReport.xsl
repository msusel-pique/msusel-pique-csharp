...
<xsl:template match="Exceptions">
		...
		<xsl:choose>
            <xsl:when test="$ExceptionCount > 0">
                <xsl:variable name="nodeId" select="generate-id()"/>
                <div class="NodeLine">
                ...
                <div class="NodeDiv" style="display: none; position: relative; padding-left: 11;">
					...
					<table width="100%">
					    ...
					    <tr>
							<td/>
							<td class="ColumnHeader">Code</td>
							<td class="ColumnHeader">Type</td>
							<td width="100%" class="ColumnHeader">Message</td>
						</tr>
...
<xsl:template match="Exception">
    <xsl:variable name="exceptionId" select="generate-id()"/>

	<tr>            
		<xsl:attribute name="onClick">
			javascript:ViewState('<xsl:value-of select="$exceptionId"/>');
		</xsl:attribute>
		...
		<td valign="top">
			<a href="javascript:DoNothing();"><nobr class="NodeIcon"><xsl:attribute name="id"><xsl:value-of select="$exceptionId"/>Toggle</xsl:attribute>&#x0034;</nobr></a>
		</td>	
		<td class="CriticalError">
	...
	<xsl:apply-templates select="StackTrace" >
		<xsl:with-param name="exceptionId"><xsl:value-of select="$exceptionId"/></xsl:with-param>
	</xsl:apply-templates>

</xsl:template>    

<xsl:template match="StackTrace">
	<xsl:param name="exceptionId"></xsl:param>
	
	<tr>
		<td/>
		<td colspan="3">    			
			<div class="MessageDiv" style="display: none; position: relative; padding-left: 5;">
				<xsl:attribute name="id">
				...
				<table width="100%" class="RuleBlock">
					<tr>
						<td class="PropertyName">Stack Trace:</td>
						<td class="PropertyContent"> 
							<xsl:value-of select="text()"/>
						</td>
					</tr>		   
